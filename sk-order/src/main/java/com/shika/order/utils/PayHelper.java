...
public class PayHelper {
    ...
    private void isSignatureValid(Map<String, String> result) {
        try {
            ...
            if (!boo1 && !boo2) {
                throw new SkException(ExceptionEnum.WX_PAY_SIGN_INVALID);
            }
        } catch (Exception e) {
    ...
    public void handleNotify(Map<String, String> msg) {
        ...
        if (StringUtils.isBlank(totalFee) || StringUtils.isBlank(outTradeNo)
            ...
            throw new SkException(ExceptionEnum.WX_PAY_NOTIFY_PARAM_ERROR);
        }
        //查询订单
        ...
        int cnt = orderStatusMapper.updateByPrimaryKeySelective(orderStatus1);
        if(cnt != 1){
